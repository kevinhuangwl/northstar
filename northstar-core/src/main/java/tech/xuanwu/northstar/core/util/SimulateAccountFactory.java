...
public class SimulateAccountFactory {
	...
	public static AccountInfo createAccount(String gatewayName) {
		...
		info.setAccountId(gatewayName + CommonConstant.SIM_TAG);
		info.setName(gatewayName + CommonConstant.SIM_TAG);
		...
		info.setGatewayId(gatewayName + CommonConstant.SIM_TAG);
		info.setHolder("user");
		return info;
	}
}
